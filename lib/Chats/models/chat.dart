...
class _ChatDetailsPageState extends State<ChatDetailsPage>
  ...
  void initState() {
    ...
    _messageAnimationController
        .forward(); // Start animation for initial messages
  }
  ...
  void _fetchChatMessages() async {
    try {
      final url = Uri.parse(
          'https://cokie-chat-api.onrender.com/chats/${widget.chatId}');
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final chatWithMessages =
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext ctx, index) {
                ...
                final messageAlign = isSender
                    ? CrossAxisAlignment.end
                    : CrossAxisAlignment.start;
                final messageColor = isSender ? Colors.lightGreen : Colors.grey;
